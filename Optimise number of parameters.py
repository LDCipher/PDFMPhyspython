...
def polynomial_fit_function(x, *args):
    af, bf, alpha = args[0], args[1], args[2]
    p_arg = args[3:]
    y = x ** alpha
    f = ((x ** af) * ((1 - x) ** bf)) * ((p_arg[0] * y ** 4) + (p_arg[1] * y ** 3) + (p_arg[2] * y ** 2) +
                                         (p_arg[3] * y) + p_arg[4])
    return f
...
def plot_data(xf_values, x_values, min_val, max_val, q_value, flavour_value, interval, n):
    ...
    popt, _ = curve_fit(polynomial_fit_function, x[mask], y[mask], p0=temp)
    ...
    # Fitted data
    fitted_x = np.array(x)
    fitted_y = polynomial_fit_function(fitted_x, *popt)
...
def main():
    ...
    params, bezier_curve = plot_data(xf_values, x_values, min_val, max_val,
                                     q_value, flavour_value, bezier_interval, 4)
